...
local function is_sane_date(exchange, year, month, day)
    if TRADING_DAYS[exchange] == nil then
        return false
    end
    if TRADING_DAYS[exchange][year] == nil then
        return false
    end
    if TRADING_DAYS[exchange][year][month] == nil then
        return false
    end
...
function mod.is_nth_trading_day(exchange, year, month, day, n)
    if is_sane_date(exchange, year, month, day) == false then
        return nil
    end
    local days = TRADING_DAYS[exchange][year][month]
    local norm_n
    if n > 0 then
        norm_n = n
    else
...
function mod.is_first_trading_day(exchange, year, month, day)
    return mod.is_nth_trading_day(exchange, year, month, day, 1)
end

function mod.is_last_trading_day(exchange, year, month, day)
    return mod.is_nth_trading_day(exchange, year, month, day, -1)
end

function mod.is_trading_day(exchange, year, month, day)
    if is_sane_date(exchange, year, month, day) == false then
        return nil
    end
    for i, dayno in ipairs(TRADING_DAYS[exchange][year][month]) do
        if day == dayno then
            return true
        end
    end
...
function mod.is_first_minute(exchange, market, hour, minute)
    if exchange == EXCHANGE.MOEX then
...
function mod.is_last_minute(exchange, market, hour, minute)
    if exchange == EXCHANGE.MOEX then
...
function mod.is_closing_auction(exchange, market, hour, minute)
    if exchange == EXCHANGE.MOEX then
        if market == MARKET.MOEX.STOCKS then
        ...
        elseif market == MARKET.MOEX.FUTS then
            return false
        elseif market == MARKET.MOEX.STOCKS_USA then
            if hour == 22 and minute == 58 then
                return true
            else
                return false
            end
...
function mod.is_opening_auction(exchange, market, hour, minute)
    if exchange == EXCHANGE.MOEX then
        if (market == MARKET.MOEX.STOCKS
                or market == MARKET.MOEX.ETFS) then
